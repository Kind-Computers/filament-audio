...
{
	const SINC_TYPE *sincA;
	const SINC_TYPE *sincB;
	int32 blendFactor;  // 0 = pure A, >0 = blend (fixed-point 0..65536)
	...
	MPT_FORCEINLINE PolyphaseInterpolation(const ModChannel &chn, const CResampler &resampler, unsigned int)
	{
		...
		constexpr int64 thresh1 = 0x130000000ll;
		constexpr int64 thresh2 = 0x180000000ll;
		const int64 absInc = std::abs(chn.increment.GetRaw());

		if(absInc >= thresh2)
		{
			sincA = resampler.gDownsample2x;
			sincB = sincA;
			blendFactor = 0;
		}
		else if(absInc >= thresh1)
		{
			sincA = resampler.gKaiserSinc;
			sincB = resampler.gDownsample13x;
			blendFactor = static_cast<int32>((absInc - thresh1) * 65536 / (thresh2 - thresh1));
		}
		else
		{
			sincA = resampler.gKaiserSinc;
	...
	MPT_FORCEINLINE void operator() (typename Traits::outbuf_t &outSample, const typename Traits::input_t * const MPT_RESTRICT inBuffer, const uint32 posLo)
	{
		...
		const SINC_TYPE *lutA = sincA + phase;
		...
		for(int i = 0; i < Traits::numChannelsIn; i++)
		{
			auto out =
				...
				+ lutA[15] * Traits::Convert(inBuffer[i + 8 * Traits::numChannelsIn])) / (1 << SINC_QUANTSHIFT);

			if(blendFactor != 0)
			{
				const SINC_TYPE *lutB = sincB + phase;
				auto outB =
					...
					+ lutB[15] * Traits::Convert(inBuffer[i + 8 * Traits::numChannelsIn])) / (1 << SINC_QUANTSHIFT);
				out = out + static_cast<decltype(out)>((outB - out) * blendFactor / 65536);
			}
