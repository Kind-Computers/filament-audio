...
int32 InstrumentEnvelope::GetValueFromPosition(int position, int32 rangeOut, int32 rangeIn) const
{
	...
	if(position >= x2)
	{
		// Case: current 'tick' is on an envelope point.
		value = static_cast<double>(at(pt).value) / rangeIn;
	} else
	{
		...
		if(pt)
		{
			...
			value = static_cast<double>(at(pt - 1).value) / rangeIn;
			x1 = at(pt - 1).tick;
		}

		if(x2 > x1 && position > x1)
		{
			// Linear interpolation: value = y1 + (y2 - y1) * (position - x1) / (x2 - x1)
			double y2 = static_cast<double>(at(pt).value) / rangeIn;
			double t = static_cast<double>(position - x1) / (x2 - x1);
			value += (y2 - value) * t;
		}
	}
	...
	return static_cast<int32>(value * rangeOut + 0.5);
}


// Double-precision variant: returns full-precision envelope value without int32 truncation.
double InstrumentEnvelope::GetValueFromPositionF(int position, double rangeOut, int32 rangeIn) const
{
	if(empty())
		return 0.0;
