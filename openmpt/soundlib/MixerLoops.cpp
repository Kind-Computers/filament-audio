...
#else
		const mixsample_t x_r = rofs * (1.0 / (1 << OFSDECAYSHIFT));
		const mixsample_t x_l = lofs * (1.0 / (1 << OFSDECAYSHIFT));
#endif
		rofs -= x_r;
		lofs -= x_l;
		pBuffer[i*2] = rofs;
...
// Cross-type overloads for plugin interop (plugins use float buffers)
void InterleaveStereo(const float * MPT_RESTRICT inputL, const float * MPT_RESTRICT inputR, mixsample_t * MPT_RESTRICT output, size_t numSamples)
{
	while(numSamples--)
	{
		*(output++) = static_cast<mixsample_t>(*(inputL++));
		*(output++) = static_cast<mixsample_t>(*(inputR++));
	}
}


void DeinterleaveStereo(const mixsample_t * MPT_RESTRICT input, float * MPT_RESTRICT outputL, float * MPT_RESTRICT outputR, size_t numSamples)
{
	while(numSamples--)
	{
		*(outputL++) = static_cast<float>(*(input++));
		*(outputR++) = static_cast<float>(*(input++));
	}
}
