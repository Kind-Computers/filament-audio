...
struct SymVirtualInst
{
	...
	bool Render(CSoundFile &sndFile, const bool asQueue, ModSample &target, uint16 sampleBoost) const
	{
		...
		const double rateFactor = 1.0 / std::max(sndFile.GetSample(events[0].inst + 1).nC5Speed, FreqT(1));
...
#else
		{
			const mixsample_t *src = buffer.data();
			int16 *dst = target.sample16() + writeOffset;
			for(SmpLength i = 0; i < writeCount; i++)
			{
				mixsample_t val = src[i * 2];
				if(val > 1.0f) val = 1.0f;
				if(val < -1.0f) val = -1.0f;
				dst[i] = mpt::saturate_round<int16>(val * 32767.0f);
			}
		}
#endif
