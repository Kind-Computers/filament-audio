...
public:
	...
	void Process(mpt::audio_span_interleaved<double> buffer) override
	{
		if constexpr(std::is_same<typename Taudio_span::sample_type, double>::value)
		{
			// Double output target: direct passthrough without float conversion or dithering
			// (dithering is only meaningful when quantizing to lower precision)
			for(std::size_t i = 0; i < buffer.size_frames(); ++i)
			{
				for(std::size_t channel = 0; channel < buffer.size_channels(); ++channel)
				{
					outputBuffer(channel, countRendered + i) = buffer(channel, i);
				}
			}
			countRendered += buffer.size_frames();
		}
		...
		{
			// Downconvert double mix buffer to float for output
			const std::size_t totalSamples = buffer.size_channels() * buffer.size_frames();
			std::vector<MixSampleFloat> floatBuf(totalSamples);
			for(std::size_t i = 0; i < totalSamples; ++i)
				floatBuf[i] = static_cast<MixSampleFloat>(buffer.data()[i]);
			Process(mpt::audio_span_interleaved<MixSampleFloat>(floatBuf.data(), buffer.size_channels(), buffer.size_frames()));
		}
	}
#endif
};
	...
	void Process(mpt::audio_span_interleaved<double> buffer) override
	{
		if(gainFactor != MixSampleFloat(1.0))
		{
			for(std::size_t frame = 0; frame < buffer.size_frames(); ++frame)
			{
				for(std::size_t channel = 0; channel < buffer.size_channels(); ++channel)
				{
					buffer(channel, frame) *= static_cast<double>(gainFactor);
				}
			}
		}
		Tbase::Process(buffer);
