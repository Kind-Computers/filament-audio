...
static somefloat32 NextTPDFDither(std::uint32_t &state, somefloat32 lsb)
{
	auto nextUniform = [&state]() -> somefloat32
	{
		state = state * 1664525u + 1013904223u;
		return static_cast<somefloat32>(state >> 8) * (1.0f / 16777216.0f);
	};
	return (nextUniform() - nextUniform()) * lsb;
}

static bool ShouldDecodeDirectToFloat(const SampleIO &sampleIO)
{
	switch(sampleIO.GetEncoding())
	{
	case SampleIO::floatPCM:
	case SampleIO::floatPCM15:
	case SampleIO::floatPCM23:
	case SampleIO::floatPCMnormalize:
		return true;
	default:
		return sampleIO.GetBitDepth() > 16;
	}
}

static bool QuantizeFloatSample(const ModSample &sample, SampleIO::Bitdepth bitDepth, ModSample &quantized)
{
	const bool isFloat32 = sample.GetRuntimeSampleFormat() == ModSample::RuntimeSampleFormat::Float32;
	const bool isFloat64 = sample.GetRuntimeSampleFormat() == ModSample::RuntimeSampleFormat::Float64;
	...
	quantized = sample;
	quantized.pData.pSample = nullptr;
	quantized.runtimeFormat = ModSample::RuntimeSampleFormat::Auto;
	quantized.uFlags.set(CHN_16BIT, bitDepth >= SampleIO::_16bit);
	quantized.SetSaveBitsPerSample(bitDepth >= SampleIO::_16bit ? 16 : 8);
	if(!quantized.AllocateSample())
	{
		return false;
	}

	const SmpLength count = sample.nLength * sample.GetNumChannels();
	const bool is16bit = bitDepth >= SampleIO::_16bit;
	std::uint32_t state = 0x51ED270Bu ^ static_cast<std::uint32_t>(sample.nLength) ^ (static_cast<std::uint32_t>(sample.GetNumChannels()) << 24);

	if(is16bit)
	{
		int16 *dst = quantized.sample16();
		if(isFloat64)
		{
			const double *src = sample.sampled();
			for(SmpLength i = 0; i < count; i++)
				dst[i] = static_cast<int16>(SampleIO::QuantizeFloatToInt32(static_cast<float>(src[i]), 16, state));
		} else
		{
			const somefloat32 *src = sample.samplef();
			for(SmpLength i = 0; i < count; i++)
				dst[i] = static_cast<int16>(SampleIO::QuantizeFloatToInt32(src[i], 16, state));
		}
	} else
	{
		int8 *dst = quantized.sample8();
		if(isFloat64)
		{
			const double *src = sample.sampled();
			for(SmpLength i = 0; i < count; i++)
				dst[i] = static_cast<int8>(SampleIO::QuantizeFloatToInt32(static_cast<float>(src[i]), 8, state));
		} else
		{
			const somefloat32 *src = sample.samplef();
			for(SmpLength i = 0; i < count; i++)
				dst[i] = static_cast<int8>(SampleIO::QuantizeFloatToInt32(src[i], 8, state));
		}
	}
...
size_t SampleIO::ReadSample(ModSample &sample, FileReader &file) const
{
	...
	const bool decodeToFloat = ShouldDecodeDirectToFloat(*this);
	sample.uFlags.set(CHN_16BIT, GetBitDepth() >= 16);
	sample.uFlags.set(CHN_STEREO, GetChannelFormat() != mono);
	sample.SetRuntimeSampleFormat(decodeToFloat ? ModSample::RuntimeSampleFormat::Float32 : ModSample::RuntimeSampleFormat::Auto);
	sample.SetSaveBitsPerSample(GetBitDepth() >= 16 ? 16 : 8);
	...
	else if(GetBitDepth() == 24 && GetChannelFormat() == mono && GetEncoding() == signedPCM)
	{
		...
		{
			bytesRead = CopyMonoSample<SC::ConversionChain<SC::Convert<somefloat32, int32>, SC::DecodeInt24<0, littleEndian24> > >(sample, sourceBuf, fileSize);
		} else
		{
			bytesRead = CopyMonoSample<SC::ConversionChain<SC::Convert<somefloat32, int32>, SC::DecodeInt24<0, bigEndian24> > >(sample, sourceBuf, fileSize);
		}
	}
	...
	else if(GetBitDepth() == 24 && GetChannelFormat() == stereoInterleaved && GetEncoding() == signedPCM)
	{
		if(GetEndianness() == littleEndian)
		{
			bytesRead = CopyStereoInterleavedSample<SC::ConversionChain<SC::Convert<somefloat32, int32>, SC::DecodeInt24<0, littleEndian24> > >(sample, sourceBuf, fileSize);
		} else
		{
			bytesRead = CopyStereoInterleavedSample<SC::ConversionChain<SC::Convert<somefloat32, int32>, SC::DecodeInt24<0, bigEndian24> > >(sample, sourceBuf, fileSize);
		}
	}
	...
	else if(GetBitDepth() == 32 && GetChannelFormat() == mono && GetEncoding() == signedPCM)
	{
		if(GetEndianness() == littleEndian)
		{
			bytesRead = CopyMonoSample<SC::ConversionChain<SC::Convert<somefloat32, int32>, SC::DecodeInt32<0, littleEndian32> > >(sample, sourceBuf, fileSize);
		} else
		{
			bytesRead = CopyMonoSample<SC::ConversionChain<SC::Convert<somefloat32, int32>, SC::DecodeInt32<0, bigEndian32> > >(sample, sourceBuf, fileSize);
		}
	}
	...
	else if(GetBitDepth() == 32 && GetChannelFormat() == stereoInterleaved && GetEncoding() == signedPCM)
	{
		if(GetEndianness() == littleEndian)
		{
			bytesRead = CopyStereoInterleavedSample<SC::ConversionChain<SC::Convert<somefloat32, int32>, SC::DecodeInt32<0, littleEndian32> > >(sample, sourceBuf, fileSize);
		} else
		{
			bytesRead = CopyStereoInterleavedSample<SC::ConversionChain<SC::Convert<somefloat32, int32>, SC::DecodeInt32<0, bigEndian32> > >(sample, sourceBuf, fileSize);
		}
	}
	...
	else if(GetBitDepth() == 64 && GetChannelFormat() == mono && GetEncoding() == signedPCM)
	{
		if(GetEndianness() == littleEndian)
		{
			bytesRead = CopyMonoSample<SC::ConversionChain<SC::Convert<somefloat32, int64>, SC::DecodeInt64<0, littleEndian64> > >(sample, sourceBuf, fileSize);
		} else
		{
			bytesRead = CopyMonoSample<SC::ConversionChain<SC::Convert<somefloat32, int64>, SC::DecodeInt64<0, bigEndian64> > >(sample, sourceBuf, fileSize);
		}
	}
	...
	else if(GetBitDepth() == 64 && GetChannelFormat() == stereoInterleaved && GetEncoding() == signedPCM)
	{
		if(GetEndianness() == littleEndian)
		{
			bytesRead = CopyStereoInterleavedSample<SC::ConversionChain<SC::Convert<somefloat32, int64>, SC::DecodeInt64<0, littleEndian64> > >(sample, sourceBuf, fileSize);
		} else
		{
			bytesRead = CopyStereoInterleavedSample<SC::ConversionChain<SC::Convert<somefloat32, int64>, SC::DecodeInt64<0, bigEndian64> > >(sample, sourceBuf, fileSize);
		}
	}
	...
	else if(GetBitDepth() == 32 && GetChannelFormat() == mono && GetEncoding() == floatPCM)
	{
		if(GetEndianness() == littleEndian)
		{
			bytesRead = CopyMonoSample<SC::ConversionChain<SC::Convert<somefloat32, somefloat32>, SC::DecodeFloat32<littleEndian32> > >(sample, sourceBuf, fileSize);
		} else
		{
			bytesRead = CopyMonoSample<SC::ConversionChain<SC::Convert<somefloat32, somefloat32>, SC::DecodeFloat32<bigEndian32> > >(sample, sourceBuf, fileSize);
		}
	}
	...
	else if(GetBitDepth() == 32 && GetChannelFormat() == stereoInterleaved && GetEncoding() == floatPCM)
	{
		if(GetEndianness() == littleEndian)
		{
			bytesRead = CopyStereoInterleavedSample<SC::ConversionChain<SC::Convert<somefloat32, somefloat32>, SC::DecodeFloat32<littleEndian32> > >(sample, sourceBuf, fileSize);
		} else
		{
			bytesRead = CopyStereoInterleavedSample<SC::ConversionChain<SC::Convert<somefloat32, somefloat32>, SC::DecodeFloat32<bigEndian32> > >(sample, sourceBuf, fileSize);
		}
	}
	...
	else if(GetBitDepth() == 64 && GetChannelFormat() == mono && GetEncoding() == floatPCM)
	{
		if(GetEndianness() == littleEndian)
		{
			bytesRead = CopyMonoSample<SC::ConversionChain<SC::Convert<somefloat32, somefloat64>, SC::DecodeFloat64<littleEndian64> > >(sample, sourceBuf, fileSize);
		} else
		{
			bytesRead = CopyMonoSample<SC::ConversionChain<SC::Convert<somefloat32, somefloat64>, SC::DecodeFloat64<bigEndian64> > >(sample, sourceBuf, fileSize);
		}
	}
	...
	else if(GetBitDepth() == 64 && GetChannelFormat() == stereoInterleaved && GetEncoding() == floatPCM)
	{
		if(GetEndianness() == littleEndian)
		{
			bytesRead = CopyStereoInterleavedSample<SC::ConversionChain<SC::Convert<somefloat32, somefloat64>, SC::DecodeFloat64<littleEndian64> > >(sample, sourceBuf, fileSize);
		} else
		{
			bytesRead = CopyStereoInterleavedSample<SC::ConversionChain<SC::Convert<somefloat32, somefloat64>, SC::DecodeFloat64<bigEndian64> > >(sample, sourceBuf, fileSize);
		}
	}
	...
	{
		// Normalize to float32 runtime
		uint32 srcPeak = uint32(1)<<31;
		if(GetEndianness() == littleEndian)
		{
			bytesRead = CopyAndNormalizeSample<SC::NormalizationChain<SC::Convert<somefloat32, int32>, SC::DecodeInt24<0, littleEndian24> > >(sample, sourceBuf, fileSize, &srcPeak);
		} else
		{
			bytesRead = CopyAndNormalizeSample<SC::NormalizationChain<SC::Convert<somefloat32, int32>, SC::DecodeInt24<0, bigEndian24> > >(sample, sourceBuf, fileSize, &srcPeak);
		}
	...
	else if(GetBitDepth() == 32 && (GetChannelFormat() == mono || GetChannelFormat() == stereoInterleaved) && GetEncoding() == signedPCMnormalize)
	{
		...
		if(GetEndianness() == littleEndian)
		{
			bytesRead = CopyAndNormalizeSample<SC::NormalizationChain<SC::Convert<somefloat32, int32>, SC::DecodeInt32<0, littleEndian32> > >(sample, sourceBuf, fileSize, &srcPeak);
		} else
		{
			bytesRead = CopyAndNormalizeSample<SC::NormalizationChain<SC::Convert<somefloat32, int32>, SC::DecodeInt32<0, bigEndian32> > >(sample, sourceBuf, fileSize, &srcPeak);
		}
		if(bytesRead && srcPeak != uint32(1)<<31)
		{
			// Adjust sample volume so we do not affect relative volume of the sample. Normalizing is only done to increase precision.
	...
	else if(GetBitDepth() == 32 && (GetChannelFormat() == mono || GetChannelFormat() == stereoInterleaved) && GetEncoding() == floatPCMnormalize)
	{
		...
		if(GetEndianness() == littleEndian)
		{
			bytesRead = CopyAndNormalizeSample<SC::NormalizationChain<SC::Convert<somefloat32, somefloat32>, SC::DecodeFloat32<littleEndian32> > >(sample, sourceBuf, fileSize, &srcPeak);
		} else
		{
			bytesRead = CopyAndNormalizeSample<SC::NormalizationChain<SC::Convert<somefloat32, somefloat32>, SC::DecodeFloat32<bigEndian32> > >(sample, sourceBuf, fileSize, &srcPeak);
		}
	...
	else if(GetBitDepth() == 64 && (GetChannelFormat() == mono || GetChannelFormat() == stereoInterleaved) && GetEncoding() == floatPCMnormalize)
	{
		...
		if(GetEndianness() == littleEndian)
		{
			bytesRead = CopyAndNormalizeSample<SC::NormalizationChain<SC::Convert<somefloat32, somefloat64>, SC::DecodeFloat64<littleEndian64> > >(sample, sourceBuf, fileSize, &srcPeak);
		} else
		{
			bytesRead = CopyAndNormalizeSample<SC::NormalizationChain<SC::Convert<somefloat32, somefloat64>, SC::DecodeFloat64<bigEndian64> > >(sample, sourceBuf, fileSize, &srcPeak);
		}
	...
	else if(GetBitDepth() == 32 && GetChannelFormat() == mono && GetEncoding() == floatPCM15)
	{
		if(GetEndianness() == littleEndian)
		{
			bytesRead = CopyMonoSample
				...
				SC::ConversionChain<SC::Convert<somefloat32, somefloat32>, SC::DecodeScaledFloat32<littleEndian32> >
				(SC::Convert<somefloat32, somefloat32>(), SC::DecodeScaledFloat32<littleEndian32>(1.0f / static_cast<float>(1<<15)))
				);
		} else
		{
			bytesRead = CopyMonoSample
				(sample, sourceBuf, fileSize,
				SC::ConversionChain<SC::Convert<somefloat32, somefloat32>, SC::DecodeScaledFloat32<bigEndian32> >
				(SC::Convert<somefloat32, somefloat32>(), SC::DecodeScaledFloat32<bigEndian32>(1.0f / static_cast<float>(1<<15)))
				);
		}
	}
	...
	else if(GetBitDepth() == 32 && GetChannelFormat() == mono && GetEncoding() == floatPCM23)
	{
		if(GetEndianness() == littleEndian)
		{
			bytesRead = CopyMonoSample
				(sample, sourceBuf, fileSize,
				SC::ConversionChain<SC::Convert<somefloat32, somefloat32>, SC::DecodeScaledFloat32<littleEndian32> >
				(SC::Convert<somefloat32, somefloat32>(), SC::DecodeScaledFloat32<littleEndian32>(1.0f / static_cast<float>(1<<23)))
				);
		} else
		{
			bytesRead = CopyMonoSample
				(sample, sourceBuf, fileSize,
				SC::ConversionChain<SC::Convert<somefloat32, somefloat32>, SC::DecodeScaledFloat32<bigEndian32> >
				(SC::Convert<somefloat32, somefloat32>(), SC::DecodeScaledFloat32<bigEndian32>(1.0f / static_cast<float>(1<<23)))
				);
		}
	}
...
size_t SampleIO::WriteSample(std::ostream &f, const ModSample &sample, SmpLength maxSamples) const
{
	...
	if(sample.GetRuntimeSampleFormat() == ModSample::RuntimeSampleFormat::Float32 && GetEncoding() != floatPCM)
	{
		ModSample quantized;
		if(!QuantizeFloatSample(sample, GetBitDepth() >= 16 ? SampleIO::_16bit : SampleIO::_8bit, quantized))
		{
			return 0;
		}
		const size_t len = WriteSample(f, quantized, maxSamples);
		quantized.FreeSample();
		return len;
	}
	...
	if(GetBitDepth() == 32 && (GetChannelFormat() == mono || GetChannelFormat() == stereoInterleaved) && GetEncoding() == floatPCM)
	{
		const uint8 numChannels = GetNumChannels();
		MPT_ASSERT(len == numSamples * numChannels * sizeof(float32le));
		for(SmpLength frame = 0; frame < numSamples; frame++)
		{
			for(uint8 chn = 0; chn < numChannels; chn++)
			{
				const somefloat32 value = sample.ReadSampleAsFloat(frame, chn);
				if(GetEndianness() == littleEndian)
				{
					mpt::IO::Write(fb, IEEE754binary32LE(value));
				} else
				{
					mpt::IO::Write(fb, IEEE754binary32BE(value));
				}
			}
		}
	}
...
int32 SampleIO::QuantizeFloatToInt32(somefloat32 sample, uint8 bitsPerSample, std::uint32_t &state)
{
	MPT_ASSERT(bitsPerSample >= 1 && bitsPerSample <= 32);
	const double negativeScale = std::ldexp(1.0, bitsPerSample - 1);
	const double positiveScale = negativeScale - 1.0;
	const somefloat32 lsb = static_cast<somefloat32>(1.0 / negativeScale);
	double value = static_cast<double>(sample) + static_cast<double>(NextTPDFDither(state, lsb));
	if(value > (positiveScale / negativeScale))
		value = positiveScale / negativeScale;
	if(value < -1.0)
		value = -1.0;
	return (value >= 0.0)
		? mpt::saturate_round<int32>(value * positiveScale)
		: mpt::saturate_round<int32>(value * negativeScale);
}
