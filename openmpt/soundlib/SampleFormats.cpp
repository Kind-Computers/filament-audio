...
bool CSoundFile::ReadSampleFromFile(SAMPLEINDEX nSample, FileReader &file, bool mayNormalize, bool includeInstrumentFormats)
{
	...
	{
		InitOPL();
	} else if(Samples[nSample].HasSampleData())
	{
		Samples[nSample].ConvertStoredDataToFloat();
...
bool CSoundFile::SaveWAVSample(SAMPLEINDEX nSample, std::ostream &f) const
{
	...
	if(sample.uFlags[CHN_ADLIB] || !sample.HasSampleData())
		return false;
	const bool saveFloat = sample.GetRuntimeSampleFormat() == ModSample::RuntimeSampleFormat::Float32;
	const bool save16bit = sample.GetSaveBitsPerSample() >= 16;

	mpt::IO::OFile<std::ostream> ff(f);
	WAVSampleWriter file(ff);

	file.WriteFormat(sample.GetSampleRate(GetType()), saveFloat ? 32 : (save16bit ? 16 : 8), sample.GetNumChannels(), saveFloat ? WAVFormatChunk::fmtFloat : WAVFormatChunk::fmtPCM);

	// Write sample data
	file.StartChunk(RIFFChunk::iddata);
	SampleIO(
		saveFloat ? SampleIO::_32bit : (save16bit ? SampleIO::_16bit : SampleIO::_8bit),
		sample.uFlags[CHN_STEREO] ? SampleIO::stereoInterleaved : SampleIO::mono,
		SampleIO::littleEndian,
		saveFloat ? SampleIO::floatPCM : (save16bit ? SampleIO::signedPCM : SampleIO::unsignedPCM))
...
bool CSoundFile::SaveRAWSample(SAMPLEINDEX nSample, std::ostream &f) const
{
	...
	const bool save16bit = sample.GetSaveBitsPerSample() >= 16;
	SampleIO(
		save16bit ? SampleIO::_16bit : SampleIO::_8bit,
		sample.uFlags[CHN_STEREO] ? SampleIO::stereoInterleaved : SampleIO::mono,
		SampleIO::littleEndian,
		SampleIO::signedPCM)
		.WriteSample(f, sample);
...
bool CSoundFile::SaveIFFSample(SAMPLEINDEX smp, std::ostream &f) const
{
	...
	const uint8 bps = sample.GetSavedBytesPerSample();
