...
struct FLACDecoder
{
	...
	static FLAC__StreamDecoderWriteStatus write_cb(const FLAC__StreamDecoder *decoder, const FLAC__Frame *frame, const FLAC__int32 *const buffer[], void *client_data)
	{
		...
		MPT_ASSERT((bps <= 8 && sample.GetRuntimeSampleFormat() == ModSample::RuntimeSampleFormat::Int8)
			|| (bps > 8 && bps <= 16 && sample.GetRuntimeSampleFormat() == ModSample::RuntimeSampleFormat::Int16)
			|| (bps > 16 && sample.GetRuntimeSampleFormat() == ModSample::RuntimeSampleFormat::Float32));
		...
		for(uint8 chn = 0; chn < modChannels; chn++)
		{
			...
			{
				const somefloat32 scale = static_cast<somefloat32>(1.0 / static_cast<double>(uint64(1) << (bps - 1)));
				somefloat32 *sampleDataFloat = sample.samplef() + offset;
				for(SmpLength frame = 0; frame < copySamples; frame++)
				{
					sampleDataFloat[frame * modChannels + chn] = static_cast<somefloat32>(buffer[chn][frame]) * scale;
				}
			}
		}
	...
	static void metadata_cb(const FLAC__StreamDecoder *, const FLAC__StreamMetadata *metadata, void *client_data)
	{
		...
		if(metadata->type == FLAC__METADATA_TYPE_STREAMINFO && metadata->data.stream_info.total_samples != 0)
		{
			...
			sample.SetRuntimeSampleFormat(metadata->data.stream_info.bits_per_sample > 16 ? ModSample::RuntimeSampleFormat::Float32 : ModSample::RuntimeSampleFormat::Auto);
			sample.SetSaveBitsPerSample(metadata->data.stream_info.bits_per_sample > 8 ? 16 : 8);
...
bool CSoundFile::SaveFLACSample(SAMPLEINDEX nSample, std::ostream &f) const
{
	...
	const uint8 bitsPerSample = sample.GetRuntimeSampleFormat() == ModSample::RuntimeSampleFormat::Float32 ? 24 : sample.GetElementarySampleSize() * 8;
	...
	std::uint32_t ditherState = 0xA35E2F19u ^ static_cast<std::uint32_t>(sample.nLength) ^ (static_cast<std::uint32_t>(numChannels) << 24);
	while(framesRemain && success)
	{
		...
		switch(sample.GetRuntimeSampleFormat())
		{
		case ModSample::RuntimeSampleFormat::Int8:
		case ModSample::RuntimeSampleFormat::Auto:
			std::copy(sample.sample8() + framesRead * numChannels, sample.sample8() + (framesRead + copyFrames) * numChannels, std::begin(buffer));
			break;
		case ModSample::RuntimeSampleFormat::Int16:
			std::copy(sample.sample16() + framesRead * numChannels, sample.sample16() + (framesRead + copyFrames) * numChannels, std::begin(buffer));
			break;
		case ModSample::RuntimeSampleFormat::Float32:
		{
			const somefloat32 *src = sample.samplef() + framesRead * numChannels;
			for(SmpLength i = 0; i < copyFrames * numChannels; i++)
			{
				buffer[i] = SampleIO::QuantizeFloatToInt32(src[i], bitsPerSample, ditherState);
			}
			break;
		}
		}
