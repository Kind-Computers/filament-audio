...
#if !MPT_COMPILER_QUIRK_FLOAT_PREFER64
	virtual void Process(mpt::audio_span_interleaved<double> buffer) { (void)buffer; }  // default no-op for double mixing
#endif
};
...
#if !MPT_COMPILER_QUIRK_FLOAT_PREFER64
	virtual void Process(mpt::audio_span_planar<double> buffer) { (void)buffer; }
#endif
};
...
#if !MPT_COMPILER_QUIRK_FLOAT_PREFER64
	virtual void Process(mpt::audio_span_planar<const double> buffer) { (void)buffer; }
#endif
};
...
#if !MPT_COMPILER_QUIRK_FLOAT_PREFER64
	virtual void Process(mpt::audio_span_interleaved<const double> buffer) { (void)buffer; }
#endif
};
...
public:
	...
	PitchT m_nMinPeriod, m_nMaxPeriod;
...
protected:
	...
	void ProcessVolumeSwing(ModChannel &chn, VolT &vol) const;
	void ProcessPanningSwing(ModChannel &chn) const;
	void ProcessTremolo(ModChannel &chn, VolT &vol) const;
	void ProcessTremor(CHANNELINDEX nChn, VolT &vol);

	bool IsEnvelopeProcessed(const ModChannel &chn, EnvelopeType env) const;
	void ProcessVolumeEnvelope(ModChannel &chn, VolT &vol) const;
	void ProcessPanningEnvelope(ModChannel &chn) const;
	int ProcessPitchFilterEnvelope(ModChannel &chn, PitchT &period) const;
	...
	void ProcessInstrumentFade(ModChannel &chn, VolT &vol) const;

	static void ProcessPitchPanSeparation(PanT &pan, int note, const ModInstrument &instr);
	void ProcessPanbrello(ModChannel &chn) const;

	void ProcessArpeggio(CHANNELINDEX nChn, PitchT &period, Tuning::NOTEINDEXTYPE &arpeggioSteps);
	void ProcessVibrato(CHANNELINDEX nChn, PitchT &period, Tuning::RATIOTYPE &vibratoFactor);
	void ProcessSampleAutoVibrato(ModChannel &chn, PitchT &period, Tuning::RATIOTYPE &vibratoFactor, int &nPeriodFrac) const;

public:
	// Public so that ReplaceSample() can immediately update chn.increment
	// after a live sample swap, eliminating partial-tick pitch glitches.
	std::pair<SamplePosition, double> GetChannelIncrement(const ModChannel &chn, PitchT period, int periodFrac) const;
...
public:
	...
	void DoFreqSlide(ModChannel &chn, PitchT &period, PitchT amount, bool isTonePorta = false) const;
	...
	double CutOffToFrequency(uint32 nCutOff, int envModifier = 256) const;
...
public:
	...
	PitchT GetNoteFromPeriod(PitchT period, int32 nFineTune = 0, FreqT nC5Speed = 0) const;
	PitchT GetPeriodFromNote(uint32 note, int32 nFineTune, FreqT nC5Speed) const;
	double GetFreqFromPeriod(PitchT period, FreqT c5speed, PitchT nPeriodFrac = 0) const;
	// Misc functions
