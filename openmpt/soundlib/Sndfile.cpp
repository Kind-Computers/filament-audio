...
bool CSoundFile::CreateInternal(FileReader file, ModLoadingFlags loadFlags)
{
	...
	for(SAMPLEINDEX nSmp = 1; nSmp <= m_nSamples; nSmp++)
	{
		...
		{
			if(!sample.ConvertStoredDataToFloat())
			{
				DestroySampleThreadsafe(nSmp);
				continue;
			}
...
bool CSoundFile::DestroySample(SAMPLEINDEX nSample)
{
	...
	sample.SetRuntimeSampleFormat(ModSample::RuntimeSampleFormat::Auto);
	sample.SetSaveBitsPerSample(8);
...
bool CSoundFile::LoadExternalSample(SAMPLEINDEX smp, const mpt::PathString &filename)
{
	...
	if(f.IsValid())
	{
		...
		if(ok)
		{
			...
			const auto newRuntimeFormat = sample.runtimeFormat;
			const uint8 newSaveBits = sample.GetSaveBitsPerSample();
			...
			sample.runtimeFormat = newRuntimeFormat;
			sample.SetSaveBitsPerSample(newSaveBits);
